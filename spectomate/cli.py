...
from spectomate import registry, __version__
...
@cli.command()
@click.option("-s", "--source-format", required=True, help="Format źródłowy (np. pip, conda, poetry)")
@click.option("-t", "--target-format", required=True, help="Format docelowy (np. pip, conda, poetry)")
@click.option("-i", "--input-file", required=True, type=click.Path(exists=True), help="Ścieżka do pliku wejściowego")
@click.option("-o", "--output-file", type=click.Path(), help="Ścieżka do pliku wyjściowego")
...
@click.option("--use-external/--no-external", default=False, help="Użyj zewnętrznych narzędzi do konwersji jeśli dostępne")
@click.option("--verbose/--quiet", default=False, help="Wyświetlaj szczegółowe informacje")
def convert(
    source_format: str,
    target_format: str,
    input_file: str,
    ...
    try:
        ...
        if converter_cls is None:
            click.echo(f"Błąd: Nie znaleziono konwertera z {source_format} do {target_format}", err=True)
            sys.exit(1)
