#!/usr/bin/env python
"""
Przykład użycia Spectomate do konwersji z formatu conda (environment.yml) do formatu pip (requirements.txt).
"""

import sys
from pathlib import Path

from spectomate.converters.conda_to_pip import CondaToPipConverter


def main():
    """
    Główna funkcja przykładu.
    """
    # Sprawdzamy, czy podano argumenty
    if len(sys.argv) < 2:
        print("Użycie: python conda_to_pip_example.py <ścieżka_do_environment.yml>")
        return 1
    
    # Pobieramy ścieżkę do pliku environment.yml
    environment_file = Path(sys.argv[1])
    
    # Sprawdzamy, czy plik istnieje
    if not environment_file.exists():
        print(f"Błąd: Plik {environment_file} nie istnieje.")
        return 1
    
    # Tworzymy ścieżkę do pliku wyjściowego
    output_file = environment_file.parent / "requirements.txt"
    
    print(f"Konwersja {environment_file} do {output_file}...")
    
    # Tworzymy konwerter
    converter = CondaToPipConverter(
        source_file=environment_file,
        target_file=output_file
    )
    
    # Wykonujemy konwersję
    try:
        result_path = converter.execute()
        print(f"Konwersja zakończona pomyślnie. Wynik zapisano w: {result_path}")
        return 0
    except Exception as e:
        print(f"Błąd podczas konwersji: {e}")
        return 1


if __name__ == "__main__":
    sys.exit(main())
